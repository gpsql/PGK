...
class Ship:
    ...
    def update(self, dt: float) -> None:
        ...
        forward_y = -math.cos(self.angle)

        self._thrusting = rl.is_key_down(rl.KeyboardKey.KEY_UP)
        if self._thrusting:
            self.vel.x += forward_x * THRUST * dt
            self.vel.y += forward_y * THRUST * dt
    ...
    def _draw_at(self, ox: float, oy: float) -> None:
        ...
        if DEBUG:
            speed = math.hypot(self.vel.x, self.vel.y)
            endpoint_x = ox + self.vel.x * 0.4
            endpoint_y = oy + self.vel.y * 0.4
            rl.draw_line_v(rl.Vector2(ox, oy), rl.Vector2(endpoint_x, endpoint_y), rl.GREEN)
            rl.draw_circle_v(rl.Vector2(endpoint_x, endpoint_y), 3, rl.GREEN)
