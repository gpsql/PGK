...
## Ruch XY: lokalny względem PathFollow3D — statek automatycznie jedzie z szyną.
## Strzał: pocisk trafia do root sceny, żeby żył niezależnie od szyny.
...
func _spawn_bullet() -> void:
	if bullet_scene == null:
		push_warning("Player: bullet_scene nie jest ustawione w inspektorze!")
		return

	var bullet: Area3D = bullet_scene.instantiate()

	# Dodajemy do root — nie do PathFollow3D!
	# Gdybyśmy dodali do PathFollow3D, pocisk poruszałby się razem z szyną
	# i nie leciałby prosto w przestrzeni globalnej.
